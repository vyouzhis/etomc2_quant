...
class dataSet():
    ...
    def getCumulativeReturns(self):
        return self.returnsAnalyzer.getCumulativeReturns();
    ...
    while(i < data.__len__()):
        ...
        if timeSeries is None:
            ...
            if type(timeSeries)== np.ndarray:
                ...
                result["time"] = timeSeries[i].strftime('%Y-%m-%d')
                result["item"] = "%.5f"%data.__getitem__(i)
                #result.append([timeSeries[i].strftime('%Y-%m-%d'),"%.5f"%data.__getitem__(i)])  #只找到该方法，挨个取出
                #print  '{0:.10f}'.format(data.__getitem__(i))
        i+=1
        arr.append(result)
        #print result
    return arr
